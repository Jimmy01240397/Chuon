...
namespace SerializationDataTranslator
{
    public partial class Form1 : Form
    {
            ...
            if (openFileDialog1.FileName != "")
            {
                ...
                try
                {
                    using (FileStream fileStream = new FileStream(openFileDialog1.FileName, FileMode.Open, FileAccess.Read))
                    {
                        ...
                        int b = 0;
                        string c = "";
                        c = SerializationData.ReadString(a);
                        richTextBox1.Text = c;
                    }
                }
                catch (Exception ee)
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog1.FileName != "")
            {
                try
                {
                    ...
                    byte[] q;
                    q = SerializationData.WriteString(richTextBox1.Text);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                byte[] vs;
                vs = SerializationData.WriteString(richTextBox1.Text);
                richTextBox2.Text = SerializationData.BytesToStringWithEnter(vs);
            }
            catch (Exception ee)
            {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                richTextBox1.Text = SerializationData.ReadString(SerializationData.StringToBytes(richTextBox2.Text));
            }
            catch (Exception ee)
            {
                richTextBox1.Text = ee.ToString();
